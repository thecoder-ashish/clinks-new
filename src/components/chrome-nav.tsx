...
function Tab({ id, label, active }: { id: SectionId; label: string; active: boolean }) {
  ...
  return (
    <HoverWrap className="relative h-9 flex items-end">
      <button
        onClick={handleClick}
        className={`relative z-10 px-4 h-9 inline-flex items-center text-sm font-medium rounded-t-xl transition-colors whitespace-nowrap ${
          active ? "text-accent-foreground" : "text-tab-inactive-fg hover:text-foreground"
        }`}
      >
        {label}
      </button>
      {active && (
        <motion.div
          layoutId="active-tab"
          className="absolute inset-x-0 bottom-0 top-0 rounded-t-xl bg-accent shadow-sm"
        ...
        {isHero ? (
          <HoverWrap key="tab" className="h-9 flex items-end">
            <motion.button
              ...
              onClick={handleClick}
              className="relative h-9 px-6 inline-flex items-center gap-2 rounded-t-xl bg-accent text-accent-foreground font-display font-semibold text-base shadow-sm cursor-pointer whitespace-nowrap"
          ...
          <HoverWrap key="pill" className="h-8 flex items-center">
            <motion.button
              layoutId="crosslinks-brand"
              onClick={handleClick}
              className="h-8 px-3 inline-flex items-center gap-2 rounded-full bg-address-bg text-foreground/80 text-xs font-mono cursor-pointer whitespace-nowrap"
              transition={{ type: "spring", stiffness: 300, damping: 30 }}
            >
  ...
  return (
    <HoverWrap>
      <a
        ...
        target="_blank"
        rel="noreferrer"
        className="h-9 px-4 inline-flex items-center rounded-full border border-accent text-accent bg-transparent text-sm font-medium transition-all duration-300 hover:bg-accent hover:text-accent-foreground shadow-sm hover:shadow-md whitespace-nowrap"
      >
        Alumni Meet '25
      </a>
    </HoverWrap>
...
export function ChromeNav() {
  ...
  return (
    <header className="fixed top-0 inset-x-0 z-40 bg-frame/85 backdrop-blur-md border-b border-border/60">
...
function MobileNav({ activeId }: { activeId: SectionId }) {
  ...
  return (
    <div className="flex items-end w-full pb-1.5">
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetTrigger asChild>
          <button className="h-10 px-4 inline-flex items-center gap-2 rounded-t-xl bg-accent text-accent-foreground text-base font-medium shadow-sm">
            {current}
        ...
        <SheetContent side="top" className="pt-10">
          <SheetHeader>
            <SheetTitle className="font-display">Crosslinks</SheetTitle>
          </SheetHeader>
          <div className="mt-6 flex flex-col gap-1">
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="mt-3 px-4 py-3 rounded-lg bg-accent text-accent-foreground text-center font-medium"
            >
              Alumni Meet '25
            </a>
